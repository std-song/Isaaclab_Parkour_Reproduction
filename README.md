...
使用相同检查点进一步测试了障碍高度、障碍宽度与间距、台阶高度与连续台阶数量、沟槽宽度和独木桥宽度。下表为路点完成率从标准难度到困难难度的变化：

| 地形变量 | 教师策略 | 学生策略 |
|---|---:|---:|
| 障碍高度、宽度和间距 | 0.98 → 0.44 | 0.91 → 0.41 |
| 台阶高度与连续台阶数量 | 0.99 → 0.92 | 0.83 → 0.73 |
| 沟槽宽度 | 1.00 → 0.65 | 0.89 → 0.49 |
| 独木桥宽度 | 0.89 → 0.16 | 0.88 → 0.03 |

完整参数、24 组评测数据、分析结论、原始日志和 24 段对应演示视频见[地形参数补充实验报告](results/terrain_supplementary/README.md)。
